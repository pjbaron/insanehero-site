...
AudioManager.prototype.play = function( _key )
{
	...
	{
		var key = Utils.randomFromList( _key );
...
{
    // (done) TODO: disabled restriction on Android+Chrome audio to get debug from play returning zero
	...
	if ( _key && this.list && this.list[ _key ] )
	{
		...
		if ( Main.muteUntil <= Main.time )
		{
            ...
            if ( Main.debug )
            {
                ...
                if ( !res ) console.log("SFX not playable!");
            }
			return sfx;
		}
...
// _delay in seconds
AudioManager.prototype.playDelayed = function( _key, _delay, _cancelCallback, _context )
{
	if (Main.isAndroid && Main.isChromeMobile) return null;

	if ( Array.isArray( _key ) )
	{
		_key = Utils.randomFromList( _key );
	}

	if ( _key && this.list && this.list[ _key ] && !this.mute )
	{
		//console.log("AudioManager.playDelayed", _key);
		this.delayed.push( { key:_key, time: Main.time + _delay * 1000, cancelCallback: _cancelCallback, context: _context } );
	}
};
...
AudioManager.prototype.update = function()
{
	...
	for( var i = this.delayed.length - 1; i >= 0; --i )
	{
		...
		if ( p && p.cancelCallback && p.cancelCallback.call( p.context, now > p.time ) )
		{
...
AudioManager.prototype.startTune = function( _key, _muted )
{
	if (Main.isAndroid && Main.isChromeMobile) return;

	if ( this.gameTune ) this.stopTune();
	this.list[ _key ].loop(true);
