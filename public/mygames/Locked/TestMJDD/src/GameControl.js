		...
		switch ( this.state )
		{
			case GameControl.GameStates.PRELOAD:
				...
				if ( this.preloader.allLoaded() )
				{
                    ...
                    // fix the actual number of game levels from loaded data
                    this.preloader.levels = this.preloader.dataManager.get("levels").levels.length;
					if ( Main.debug )
                    {
                        console.log("Data has " + this.preloader.levels + " levels.");
                    }
...
//					this.preloader.audioManager.startTune( "game_tune" );
				}
				break;
			...
			case GameControl.GameStates.PLAY:
				if ( newState )
				{
					...
					this.resetInput();
					this.game = new Game( );
					this.game.create( this.preloader.getManagers(), this.preloader.levels );
				}
