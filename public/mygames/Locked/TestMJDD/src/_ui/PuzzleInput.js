...
PuzzleInput.prototype.update = function()
{
	...
	if ( this.spriteList && this.spriteList.length > 0 )
	{
		for( var i = this.spriteList.length - 1; i >= 0; --i )
		{
            ...
            else if ( s instanceof PIXI.Container )
            {
                s.y -= 4;
...
PuzzleInput.prototype.showDropAndHoverPieces = function()
{
	...
	if ( this.dropPiece )
	{
		...
		this.dropPiece.sprite.y = World.SelectionPreviewY;
	...
	if ( this.hoveredPiece )
	{
		...
		this.hoveredPiece.sprite.y = World.SelectionPreviewY;
...
PuzzleInput.prototype._selectPiece = function( _piece )
{
	...
	sprite.create( Main.topUI, "cube_" + _piece.symbol + "_1", this.managers.textures, World.SelectionPreviewX, World.SelectionPreviewY, false );
...
PuzzleInput.prototype._chainBonusScoring = function( _piece )
{
	...
	if ( this.lastMatchTime !== -1 && Main.time <= this.lastMatchTime + this.puzzle.speedMatchTime * 1000 )
	{
        ...
        _piece.parent.addChild(c);
        c.x = _piece.x;
        c.y = _piece.y;
        c.tz = -1000;
        if ( Main.lowResolutionAssets )
            c.scale.set(0.5);
        if ( this.spriteList )
            this.spriteList.push(c);

		var s = new Sprite();
		// _parent, _key, _textureManager, _x, _y, _pcnt
		s.create( c, "speedmatch", this.managers.textures, 0,0, false );
		s.anchor.set( 0.5 );
        s.tz = -1000;
        if ( this.spriteList )
		...
		s.create( c, digit1, this.managers.textures, 0 + 156 - 308/2 + 55/2, 0 + 39 - 157/2 + 48/2, false );
		s.anchor.set( 0.5 );
        s.tz = -1000;
		if ( this.spriteList )
			this.spriteList.push(s);
		if ( digit2 )
		{
			s = new Sprite();
			// 156 = offset of tl corner from tl corner of parent, 308 = width parent, 55 = width child
			s.create( c, digit2, this.managers.textures, 0 + 156 - 308/2 + 55/2 + 48, 0 + 39 - 157/2 + 48/2, false );
			s.anchor.set( 0.5 );
            s.tz = -1000;
			if ( this.spriteList )
				this.spriteList.push(s);
...
PuzzleInput.prototype._multiBonusScoring = function( _piece )
{
	...
	if ( _piece.symbol == this.lastMatchType )
	{
		...
		var s = new Sprite();
		// _parent, _key, _textureManager, _x, _y, _pcnt
		s.create( c, "multimatch", this.managers.textures, 0, 0 + 150, false );
		s.anchor.set( 0.5 );
        s.tz = -1000;
		if ( this.spriteList )
			this.spriteList.push(s);
		...
		s.create( c, digit1, this.managers.textures, 0 + 146 - 286/2 + 55/2, 0 + 150 + 47 - 161/2 + 48/2, false );
		s.anchor.set( 0.5 );
        s.tz = -1000;
		if ( this.spriteList )
			this.spriteList.push(s);
		if ( digit2 )
		{
			s = new Sprite();
			// 146 = offset of tl corner from tl corner of parent, 286 = width parent, 55 = width child
			s.create( c, digit2, this.managers.textures, 0 + 146 - 286/2 + 55/2 + 48, 0 + 150 + 47 - 161/2 + 48/2, false );
			s.anchor.set( 0.5 );
            s.tz = -1000;
			if ( this.spriteList )
				this.spriteList.push(s);
...
PuzzleInput.prototype._timeBonusScoring = function( _piece, _timeBonus )
{
    ...
    var s = new Sprite();
    // _parent, _key, _textureManager, _x, _y, _pcnt
    s.create( c, "timematch", this.managers.textures, 0, 0 + 150, false );
    s.anchor.set( 0.5 );
    s.tz = -1000;
    ...
    s.create( c, digit1, this.managers.textures, 0 + 75 - 286/2 + 55/2, 0 + 140 + 47 - 161/2 + 48/2, false );
    s.anchor.set( 0.5 );
    s.tz = -1000;
    s.alpha = 1.2;
    if ( this.spriteList )
    ...
    s = new Sprite();
    s.create( c, "colon", this.managers.textures, 0 + 75 - 286/2 + 55/2 + 40, 0 + 140 + 47 - 161/2 + 48/2, false );
    s.anchor.set( 0.5 );
    s.tz = -1000;
    s.alpha = 1.2;
    if ( this.spriteList )
    ...
    s = new Sprite();
    s.create( c, digit2, this.managers.textures, 0 + 75 - 286/2 + 55/2 + 40 + 40, 0 + 140 + 47 - 161/2 + 48/2, false );
    s.anchor.set( 0.5 );
    s.tz = -1000;
    s.alpha = 1.2;
    if ( this.spriteList )
    ...
    s = new Sprite();
    s.create( c, digit3, this.managers.textures, 0 + 75 - 286/2 + 55/2 + 40 + 40 + 52, 0 + 140 + 47 - 161/2 + 48/2, false );
    s.anchor.set( 0.5 );
    s.tz = -1000;
    s.alpha = 1.2;
    if ( this.spriteList )
