...
{
	return [ px * this.m[ 0 ] + py * this.m[ 2 ] + this.m[ 4 ], px * this.m[ 1 ] + py * this.m[ 3 ] + this.m[ 5 ] ]
};
...
var anm = function( _window )
{
	...
	{
		return _window.requestAnimationFrame || _window.webkitRequestAnimationFrame || _window.mozRequestAnimationFrame || _window.oRequestAnimationFrame || _window.msRequestAnimationFrame || _window.__anm__frameGen || function( callback )
		{
			return _window.setTimeout( callback, 1e3 / 60 )
		}
	};
	var __clearFrameFunc = function()
	{
		return _window.cancelAnimationFrame || _window.webkitCancelAnimationFrame || _window.mozCancelAnimationFrame || _window.oCancelAnimationFrame || _window.msCancelAnimationFrame || _window.__anm__frameRem || function( id )
		{
	...
	{
		return "rgba(" + Math.floor( r ) + "," + Math.floor( g ) + "," + Math.floor( b ) + "," + ( typeof a !== "undefined" ? a : 1 ) + ")"
	}
	...
	function ajax( url, callback )
	{
		...
		var whenDone = function()
		{
			if ( req.readyState == 4 )
			{
				...
				{
					throw new SysErr( "AJAX request for " + url + " returned " + req.status + " instead of 200" )
				}
			}
		};
	...
	function getPxRatio()
	{
		return _window.devicePixelRatio || 1
	}
	...
	var __finite = isFinite || Number.isFinite || function( n )
	{
		return n !== Infinity
	};
	var __nan = isNaN || Number.isNaN || function( n )
	{
		n !== NaN
	};
	...
	{
		return typeof Builder !== "undefined" && obj instanceof Builder
	}
	...
	function __close( n1, n2, precision )
	{
		...
		return Math.round( n1 * multiplier ) == Math.round( n2 * multiplier )
	}
	...
	{
		var _producer = _constructor ? function( message )
		{
	...
	C.M_CONTROLS_DISABLED = 2;
	C.M_INFO_ENABLED = 4;
	C.M_INFO_DISABLED = 8;
	C.M_HANDLE_EVENTS = 16;
	C.M_DO_NOT_HANDLE_EVENTS = 32;
	C.M_DRAW_STILL = 64;
	C.M_DO_NOT_DRAW_STILL = 128;
	C.M_INFINITE_DURATION = 256;
	C.M_FINITE_DURATION = 512;
	...
	C.M_VIDEO = C.M_CONTROLS_ENABLED | C.M_INFO_ENABLED | C.M_DO_NOT_HANDLE_EVENTS | C.M_DRAW_STILL | C.M_FINITE_DURATION;
	...
	__reg_event( "XT_MOUSE", "mouse", C.X_MCLICK | C.X_MDCLICK | C.X_MUP | C.X_MDOWN | C.X_MMOVE | C.X_MOVER | C.X_MOUT );
	__reg_event( "X_KPRESS", "kpress", 128 );
	__reg_event( "X_KUP", "kup", 256 );
	__reg_event( "X_KDOWN", "kdown", 1024 );
	__reg_event( "XT_KEYBOARD", "keyboard", C.X_KPRESS | C.X_KUP | C.X_KDOWN );
	...
	Player.PEFF = 0;
	Player.NO_TIME = -1;
	Player.MARKER_ATTR = "anm-player";
	Player.URL_ATTR = "data-url";
	Player.DEFAULT_CANVAS = {
	...
	Player.prototype.load = function( arg1, arg2, arg3, arg4 )
	{
		...
		if ( player.state.happens === C.PLAYING || player.state.happens === C.PAUSED )
		{
		...
		if ( object )
		{
			...
			else if ( __arr( object ) )
			{
				L.loadClips( player, object, whenDone )
			}
			else if ( __str( object ) )
			{
	...
	Player.prototype.play = function( from, speed, stopAfter )
	{
		...
		state.__firstReq = __r_loop( player.ctx, state, scene, player.__beforeFrame( scene ), player.__afterFrame( scene ), player.__userBeforeRender, player.__userAfterRender );
	...
	Player.prototype.stop = function()
	{
		...
		player._ensureHasState();
		var state = player.state;
		if ( state.happens === C.PLAYING || state.happens === C.PAUSED )
		{
	...
	{
		var cvs_opts = Player._mergeOpts( Player._optsFromCvsAttrs( this.canvas ), Player.DEFAULT_CONFIGURATION );
	...
	Player.prototype._rectChanged = function( rect )
	{
		...
		return cur.width != rect.width || cur.height != rect.height || cur.x != rect.x || cur.y != rect.y
	};
	Player.prototype.forceRedraw = function()
	{
		...
		switch ( this.state.happens )
		{
			case C.STOPPED:
				...
				break;
			case C.PAUSED:
				if ( this.anim ) this.drawAt( this.state.time );
				break;
			case C.PLAYING:
				if ( this.anim )
				{
					this._stopAndContinue()
				}
				break;
			case C.NOTHING:
				this._drawSplash();
	...
	Player.prototype.drawAt = function( time )
	{
		...
		this.anim.reset();
		__r_at( time, this.ctx, this.state, this.anim, this.__userBeforeRender, this.__userAfterRender );
		if ( this.controls )
		{
	...
	{
		return canvas.getAttribute( Player.MARKER_ATTR ) == null || canvas.getAttribute( Player.MARKER_ATTR ) == undefined
	};
	...
	Player.prototype.subscribeEvents = function( canvas )
	{
		...
		this.canvas.addEventListener( "mouseover", function( player )
		{
			return function( evt )
			{
				if ( global_opts.autoFocus && player.mode & C.M_HANDLE_EVENTS && player.canvas )
				{
					player.canvas.focus()
				}
		...
		this.canvas.addEventListener( "mouseout", function( player )
		{
			return function( evt )
			{
				...
				if ( player.state.happens === C.NOTHING ) return;
				if ( player.controls && !player.controls.evtInBounds( evt ) )
				{
					player.controls.hide()
				}
				if ( player.info && !player.info.evtInBounds( evt ) )
				{
					player.info.hide()
				}
				return true
	...
	Player.prototype._drawSplash = function()
	{
		...
		ctx.fillText( "Â© Animatron Player", 20, h - 20 );
		ctx.lineWidth = 12;
		ctx.strokeStyle = "#fee";
	...
	Player.prototype._reset = function()
	{
		...
		this.ctx.clearRect( 0, 0, state.width * state.ratio, state.height * state.ratio )
	};
	...
	Player.prototype.__beforeFrame = function( scene )
	{
		return function( player, state, scene, callback )
		{
			return function( time )
			{
				...
				if ( state.stop !== Player.NO_TIME && time >= state.from + state.stop || time > state.duration + Player.PEFF )
				{
					state.time = 0;
					scene.reset();
					player.stop();
					...
					else if ( !( player.mode & C.M_INFINITE_DURATION ) && __finite( state.duration ) )
					{
	...
	Player.prototype.__onerror = function( err )
	{
		...
		{
			throw new SysErr( _strf( Errors.S.ERROR_HANDLING_FAILED, [ err.message || err ] ) )
		}
	...
	{
		var gcs = document.defaultView && document.defaultView.getComputedStyle;
		var cpl = gcs ? parseInt( gcs( cvs, null ).paddingLeft, 10 ) || 0 : 0,
			cpt = gcs ? parseInt( gcs( cvs, null ).paddingTop, 10 ) || 0 : 0,
			cbl = gcs ? parseInt( gcs( cvs, null ).borderLeftWidth, 10 ) || 0 : 0,
			cbt = gcs ? parseInt( gcs( cvs, null ).borderTopWidth, 10 ) || 0 : 0;
	...
	{
		return canvas.hasAttribute( attr ) ? canvas.getAttribute( attr ) : _default
	}
	Player._mergeOpts = function( what, where )
	{
		var res = _mrg_obj( what, where );
		res.meta = what.meta ? _mrg_obj( what.meta, where.meta ||
		{} ) : where.meta ||
		{};
		res.anim = what.anim ? _mrg_obj( what.anim, where.anim ||
		{} ) : where.anim ||
		{};
		return res
	};
	...
	{
		var width, height, pxRatio = getPxRatio();
		return {
			debug: __attrOr( canvas, "data-debug", undefined ),
			inParent: undefined,
			...
			meta:
			{
				title: __attrOr( canvas, "data-title", undefined ),
			...
			{
				fps: undefined,
				width: __attrOr( canvas, "data-width", ( width = __attrOr( canvas, "width", undefined ), width ? width / pxRatio : undefined ) ),
				height: __attrOr( canvas, "data-height", ( height = __attrOr( canvas, "height", undefined ), height ? height / pxRatio : undefined ) ),
	...
	Player._optsFromUrlParams = function( params )
	{
		return {
			debug: params.debug,
			inParent: undefined,
			...
			anim:
			{
				...
				bgcolor:
				{
					color: params.bg ? "#" + params.bg : null
				},
				duration: undefined
			}
		}
	...
	provideEvents( Scene, [ C.X_MCLICK, C.X_MDCLICK, C.X_MUP, C.X_MDOWN, C.X_MMOVE, C.X_MOVER, C.X_MOUT, C.X_KPRESS, C.X_KUP, C.X_KDOWN, C.X_DRAW ] );
	...
	Element.ALL_MODIFIERS = [ Element.SYS_MOD, Element.TWEEN_MOD, Element.USER_MOD, Element.EVENT_MOD ];
	Element.NOEVT_MODIFIERS = [ Element.SYS_MOD, Element.TWEEN_MOD, Element.USER_MOD ];
	Element.SYS_PNT = 0;
	Element.USER_PNT = 1;
	Element.DEBUG_PNT = 2;
	Element.FIRST_PNT = Element.SYS_PNT;
	Element.LAST_PNT = Element.DEBUG_PNT;
	Element.ALL_PAINTERS = [ Element.SYS_PNT, Element.USER_PNT, Element.DEBUG_PNT ];
	...
	function Element( draw, onframe )
	{
		...
		this._modifiers = [];
		this._painters = [];
		if ( onframe ) this.__modify(
		{
			type: Element.USER_MOD
		}, onframe );
		if ( draw ) this.__paint(
		{
			type: Element.USER_PNT
		}, draw );
	...
	provideEvents( Element, [ C.X_MCLICK, C.X_MDCLICK, C.X_MUP, C.X_MDOWN, C.X_MMOVE, C.X_MOVER, C.X_MOUT, C.X_KPRESS, C.X_KUP, C.X_KDOWN, C.X_DRAW ] );
	...
	Element.prototype.render = function( ctx, gtime )
	{
		...
		if ( drawMe )
		{
			drawMe = this.fits( ltime ) && this.onframe( ltime ) && this.prepare()
		}
		if ( drawMe )
		{
			...
			else
			{
				...
				bctx.save();
				bctx.clearRect( 0, 0, dbl_scene_width, dbl_scene_height );
				bctx.save();
				...
				mctx.save();
				mctx.clearRect( 0, 0, dbl_scene_width, dbl_scene_height );
				mctx.translate( scene_width, scene_height );
				this.__mask.render( mctx, gtime );
				mctx.restore();
				bctx.drawImage( mcvs, 0, 0, dbl_scene_width, dbl_scene_height );
				bctx.restore();
				ctx.drawImage( bcvs, -scene_width, -scene_height, dbl_scene_width, dbl_scene_height )
			}
		}
		this.visible = drawMe;
		ctx.restore();
	...
	Element.prototype.addModifier = function( modifier, easing, data, priority )
	{
		if ( __obj( modifier ) )
		{
			modifier.type = Element.USER_MOD;
	...
	{
		if ( this.parent.__unsafeToRemove || this.__unsafeToRemove ) throw new AnimErr( Errors.A.UNSAFE_TO_REMOVE );
		this.parent = null;
	...
	Element.prototype.ltime = function( gtime )
	{
		...
		switch ( x.mode )
		{
			...
			case C.R_STAY:
				return __t_cmp( gtime, x.gband[ 1 ] ) <= 0 ? this.__checkJump( gtime - x.gband[ 0 ] ) : this.__checkJump( x.lband[ 1 ] - x.lband[ 0 ] );
			case C.R_LOOP:
				{
					var p = this.parent,
						px = p ? p.xdata : null;
					var durtn = x.lband[ 1 ] - x.lband[ 0 ],
						pdurtn = p ? px.lband[ 1 ] - px.lband[ 0 ] : durtn;
					if ( durtn < 0 ) return -1;
	...
	Element.prototype.clear = function()
	{
		...
		if ( !this.rendering )
		{
			var children = this.children;
			this.children = [];
			iter( children ).each( function( elm )
			{
				elm._unbind()
			} )
		}
	...
	Element.prototype.__adaptModTime = function( ltime, conf, state, modifier )
	{
		...
		if ( time == null )
		{
			_tpair = [ relative ? __adjust( ltime ) / __adjust( elm_duration ) : __adjust( ltime ), __adjust( elm_duration ) ]
		}
		else if ( __arr( time ) )
		{
			var band = time;
			if ( !relative )
			{
				...
				_tpair = [ __adjust( ltime - band[ 0 ] ), __adjust( mod_duration ) ]
			}
			else
			{
				var abs_band = [ band[ 0 ] * elm_duration, band[ 1 ] * elm_duration ];
				...
				_tpair = [ __adjust( ltime - abs_band[ 0 ] ) / __adjust( mod_duration ), __adjust( mod_duration ) ]
			}
		}
		else if ( __num( time ) )
		{
		...
		else _tpair = [ relative ? __adjust( ltime ) / __adjust( elm_duration ) : __adjust( ltime ), __adjust( elm_duration ) ];
	...
	Element.prototype.__callModifiers = function( order, ltime )
	{
		return function( elm )
		{
			...
			if ( !elm.__forAllModifiers( order, function( modifier, conf )
				{
	...
	Element.prototype.__callPainters = function( order, ctx )
	{
		! function( elm )
		{
			elm.__forAllPainters( order, function( painter, conf )
			{
	...
	Element.prototype.__addTypedModifier = function( conf, modifier )
	{
		...
		modifiers[ type ][ priority ].push( [ modifier,
		{
			type: type,
			priority: priority,
			time: conf.time,
		...
		modifier.__m_ids[ elm_id ] = type << Element.TYPE_MAX_BIT | priority << Element.PRRT_MAX_BIT | modifiers[ type ][ priority ].length - 1;
		return modifier
	};
	Element.prototype.__modify = Element.prototype.__addTypedModifier;
	Element.prototype.__forAllModifiers = function( order, f, before_type, after_type )
	{
		var modifiers = this._modifiers;
		var type, seq, cur;
		for ( var typenum = 0, last = order.length; typenum < last; typenum++ )
		{
			type = order[ typenum ];
			seq = modifiers[ type ];
			...
			if ( seq )
			{
				for ( var pi = 0, pl = seq.length; pi < pl; pi++ )
				{
					if ( cur = seq[ pi ] )
					{
	...
	Element.prototype.__addTypedPainter = function( conf, painter )
	{
		...
		painters[ type ][ priority ].push( [ painter,
		{
			type: type,
			priority: priority,
			data: conf.data
		} ] );
		painter.__p_ids[ elm_id ] = type << Element.TYPE_MAX_BIT | priority << Element.PRRT_MAX_BIT | painters[ type ][ priority ].length - 1;
		return painter
	};
	Element.prototype.__paint = Element.prototype.__addTypedPainter;
	...
	Element.prototype.__checkJump = function( at )
	{
		...
		t = s.p !== null ? s.p : null;
		t = t === null && s.t !== null ? s.t * duration : t;
		t = t === null && s.key !== null ? x.keys[ s.key ] : t;
		if ( t !== null )
		{
			if ( t < 0 || t > duration )
			{
	...
	Element.createXData = function( owner )
	{
		return {
			...
			mode: C.R_ONCE,
			nrep: Infinity,
			lband: [ 0, Element.DEFAULT_LEN ],
			gband: [ 0, Element.DEFAULT_LEN ],
			keys:
			{},
			tf: null,
			acomp: null,
	...
	Element.__convertEasing = function( easing, data, relative )
	{
		...
		if ( __str( easing ) )
		{
			...
			return relative ? f : function( t, len )
			{
				return f( t / len, len ) * len
			}
		}
	...
	{
		var _t = m ? ( m.reset(), m ) : new Transform;
	...
	{
		subj.prototype._initHandlers = function( evts )
		{
			return function()
			{
				var _hdls = {};
		...
		{
			if ( !this.provides( event ) ) throw new AnimErr( "Event '" + C.__enmap[ event ] + "' not provided by " + this );
			if ( !handler ) throw new AnimErr( "You are trying to assign " + "undefined handler for event " + event );
		...
		{
			if ( !this.provides( event ) ) throw new AnimErr( "Event " + event + " not provided by " + this );
	...
	{
		return {
			key: e.keyCode != null ? e.keyCode : e.which,
			ch: e.charCode
		}
	}
	...
	{
		return {
			pos: [ e.pageX - cvs.__rOffsetLeft, e.pageY - cvs.__rOffsetTop ]
		}
	}
	var L = {};
	...
	L.loadScene = function( player, scene, callback )
	{
		...
		if ( player.state.debug && !global_opts.liveDebug ) scene.visitElems( Element.__addDebugRender );
		player.anim = scene;
		if ( player.state.duration == undefined )
		{
			...
			{
				if ( player.mode & C.M_INFINITE_DURATION )
				{
					_duration = Infinity
				}
				else
	...
	function __r_at( time, ctx, pl_state, scene, before, after )
	{
		...
		if ( ratio != 1 ) ctx.scale( ratio, ratio );
		var size_differs = pl_state.width != scene.width || pl_state.height != scene.height;
		if ( !size_differs )
		{
			ctx.clearRect( 0, 0, scene.width, scene.height );
			if ( before ) before( time, ctx );
			scene.render( ctx, time, pl_state.zoom );
			if ( after ) after( time, ctx );
			ctx.restore()
		}
		else
		{
			__r_with_ribbons( ctx, pl_state.width, pl_state.height, scene.width, scene.height, function( _scale )
			{
				ctx.clearRect( 0, 0, scene.width, scene.height );
				if ( before ) before( time, ctx );
				scene.render( ctx, time, pl_state.zoom );
	...
	function __r_fps( ctx, fps, time )
	{
		...
		ctx.fillText( Math.floor( time * 1e3 ) / 1e3, 8, 35 )
	}
	...
	{
		if ( band[ 0 ] > duration * time ) return false;
		if ( band[ 1 ] < duration * time ) return false
	};
	...
	{
		var x = elm.xdata;
		var in_band = in_band || ( elm.parent ? elm.parent.xdata.gband : [ 0, 0 ] );
		x.gband = [ in_band[ 0 ] + x.lband[ 0 ], in_band[ 0 ] + x.lband[ 1 ] ];
	...
	Bands.wrap = function( outer, inner )
	{
		...
		return [ outer[ 0 ] + inner[ 0 ], outer[ 0 ] + inner[ 1 ] <= outer[ 1 ] ? outer[ 0 ] + inner[ 1 ] : outer[ 1 ] ]
	};
	...
	Tweens[ C.T_SCALE ] = function()
	{
		return function( t, duration, data )
		{
			this.sx = data[ 0 ][ 0 ] * ( 1 - t ) + data[ 1 ][ 0 ] * t;
			this.sy = data[ 0 ][ 1 ] * ( 1 - t ) + data[ 1 ][ 1 ] * t
		}
	};
	Tweens[ C.T_ROT_TO_PATH ] = function()
	{
		return function( t, duration, data )
		{
	...
	C.E_CSEG = "CSEG";
	var EasingImpl = {};
	EasingImpl[ C.E_PATH ] = function( path )
	{
		return function( t )
		{
	...
	EasingImpl[ C.E_FUNC ] = function( f )
	{
		return f
	};
	EasingImpl[ C.E_CSEG ] = function( seg )
	{
		return function( t )
		{
	...
	__registerSegEasing( "DEF", [ .25, .1, .25, 1, 1, 1 ] );
	__registerSegEasing( "IN", [ .42, 0, 1, 1, 1, 1 ] );
	__registerSegEasing( "OUT", [ 0, 0, .58, 1, 1, 1 ] );
	__registerSegEasing( "INOUT", [ .42, 0, .58, 1, 1, 1 ] );
	__registerSegEasing( "SIN", [ .47, 0, .745, .715, 1, 1 ] );
	__registerSegEasing( "SOUT", [ .39, .575, .565, 1, 1, 1 ] );
	__registerSegEasing( "SINOUT", [ .445, .05, .55, .95, 1, 1 ] );
	__registerSegEasing( "QIN", [ .55, .085, .68, .53, 1, 1 ] );
	__registerSegEasing( "QOUT", [ .25, .46, .45, .94, 1, 1 ] );
	__registerSegEasing( "QINOUT", [ .455, .03, .515, .955, 1, 1 ] );
	__registerSegEasing( "CIN", [ .55, .055, .675, .19, 1, 1 ] );
	__registerSegEasing( "COUT", [ .215, .61, .355, 1, 1, 1 ] );
	__registerSegEasing( "CINOUT", [ .645, .045, .355, 1, 1, 1 ] );
	__registerSegEasing( "QTIN", [ .895, .03, .685, .22, 1, 1 ] );
	__registerSegEasing( "QTOUT", [ .165, .84, .44, 1, 1, 1 ] );
	__registerSegEasing( "QTINOUT", [ .77, 0, .175, 1, 1, 1 ] );
	__registerSegEasing( "QIIN", [ .755, .05, .855, .06, 1, 1 ] );
	__registerSegEasing( "QIOUT", [ .23, 1, .32, 1, 1, 1 ] );
	__registerSegEasing( "QIINOUT", [ .86, 0, .07, 1, 1, 1 ] );
	__registerSegEasing( "EIN", [ .95, .05, .795, .035, 1, 1 ] );
	__registerSegEasing( "EOUT", [ .19, 1, .22, 1, 1, 1 ] );
	__registerSegEasing( "EINOUT", [ 1, 0, 0, 1, 1, 1 ] );
	__registerSegEasing( "CRIN", [ .6, .04, .98, .335, 1, 1 ] );
	__registerSegEasing( "CROUT", [ .075, .82, .165, 1, 1, 1 ] );
	__registerSegEasing( "CRINOUT", [ .785, .135, .15, .86, 1, 1 ] );
	__registerSegEasing( "BIN", [ .6, -.28, .735, .045, 1, 1 ] );
	__registerSegEasing( "BOUT", [ .175, .885, .32, 1.275, 1, 1 ] );
	__registerSegEasing( "BINOUT", [ .68, -.55, .265, 1.55, 1, 1 ] );
	C.P_MOVETO = 0;
	C.P_LINETO = 1;
	C.P_CURVETO = 2;
	C.PC_ROUND = "round";
	...
	{
		var p = this;
		Path.applyF( ctx, p.fill || Path.DEFAULT_FILL, p.stroke || Path.DEFAULT_STROKE, function()
		{
			p.visit( Path._applyVisitor, ctx )
		} )
	};
	Path.prototype.cstroke = function( color, width, cap, join )
	{
		this.stroke = {
			width: width != null ? width : Path.DEFAULT_STROKE.width,
			color: color,
	...
	Path.prototype.start = function()
	{
		...
		return [ this.segs[ 0 ].pts[ 0 ], this.segs[ 0 ].pts[ 1 ] ]
	};
	Path.prototype.end = function()
	{
		if ( this.segs.length < 1 ) return null;
		var lastidx = this.segs.length - 1;
		var s = this.segs[ lastidx ].count;
		return [ this.segs[ lastidx ].pts[ s - 2 ], this.segs[ lastidx ].pts[ s - 1 ] ]
	};
	Path.prototype.bounds = function()
	{
		if ( this.segs.length <= 0 ) return [ 0, 0, 0, 0 ];
		var minX = this.segs[ 0 ].pts[ 0 ],
			maxX = this.segs[ 0 ].pts[ 0 ],
			minY = this.segs[ 0 ].pts[ 1 ],
			maxY = this.segs[ 0 ].pts[ 1 ];
		this.visit( function( segment )
		{
			var pts = segment.pts,
				pnum = pts.length;
	...
	Path._strApplyVisitor = function( marker, positions, ctx )
	{
		...
		{
			ctx.bezierCurveTo( positions[ 0 ], positions[ 1 ], positions[ 2 ], positions[ 3 ], positions[ 4 ], positions[ 5 ] )
		}
	};
	...
	Path._encodeVisitor = function( segment, buffer )
	{
		...
		{
			buffer.push( "C" + positions[ 0 ] + " " + positions[ 1 ] + " " + positions[ 2 ] + " " + positions[ 3 ] + " " + positions[ 4 ] + " " + positions[ 5 ] )
		}
	};
	...
	{
		return Math.atan2( this.pts[ 1 ] - start[ 1 ], this.pts[ 0 ] - start[ 0 ] )
	};
	...
	CSeg.prototype.atT = function( start, t )
	{
		var tt = t * t,
			...
			tt3 = 3 * t * tt1,
			tt4 = 3 * tt * t1;
		return [ start[ 0 ] * tt2 + this.pts[ 0 ] * tt3 + this.pts[ 2 ] * tt4 + this.pts[ 4 ] * ttt, start[ 1 ] * tt2 + this.pts[ 1 ] * tt3 + this.pts[ 3 ] * tt4 + this.pts[ 5 ] * ttt ]
	};
	...
	CSeg.prototype.tangentAt = function( start, t )
	{
		...
		var tt = t * t;
		var p = [ 3 * par[ 0 ] * tt + 2 * par[ 1 ] * t + par[ 2 ], 3 * par[ 4 ] * tt + 2 * par[ 5 ] * t + par[ 6 ] ];
		return Math.atan2( p[ 1 ], p[ 0 ] )
	};
	CSeg.prototype._ensure_params = function( start )
	{
		if ( this._lstart && this._lstart[ 0 ] === start[ 0 ] && this._lstart[ 1 ] === start[ 1 ] ) return;
		this._lstart = start;
	...
	CSeg.prototype._calc_params = function( start )
	{
		...
		var p3x = pts[ 4 ];
		var p3y = pts[ 5 ];
		params[ 0 ] = p3x - 3 * p2x + 3 * p1x - p0x;
		params[ 1 ] = 3 * p2x - 6 * p1x + 3 * p0x;
		params[ 2 ] = 3 * p1x - 3 * p0x;
		params[ 3 ] = p0x;
		params[ 4 ] = p3y - 3 * p2y + 3 * p1y - p0y;
		params[ 5 ] = 3 * p2y - 6 * p1y + 3 * p0y;
		params[ 6 ] = 3 * p1y - 3 * p0y;
		params[ 7 ] = p0y;
		return params
	};

	function Text( lines, font, fill, stroke )
	{
		this.lines = lines;
	...
	Text.prototype.dimen = function()
	{
		...
		return this._dimen = [ buff.offsetWidth, buff.offsetHeight ]
	};
	...
	Text._createBuffer = function()
	{
		...
		_div.style.top = -1e4 + "px";
		_div.style.left = -1e4 + "px";
	...
	{
		var c = new Text( this.lines, this.font, this.fill, this.stroke );
	...
	Brush._create = function( ctx, brush )
	{
		...
		if ( brush.lgrad )
		{
			var src = brush.lgrad,
				...
				dir = src.dir,
				bounds = src.bounds;
			var grad = bounds ? ctx.createLinearGradient( bounds[ 0 ] + dir[ 0 ][ 0 ] * bounds[ 2 ], bounds[ 1 ] + dir[ 0 ][ 1 ] * bounds[ 3 ], bounds[ 0 ] + dir[ 1 ][ 0 ] * bounds[ 2 ], bounds[ 1 ] + dir[ 1 ][ 1 ] * bounds[ 3 ] ) : ctx.createLinearGradient( dir[ 0 ][ 0 ], dir[ 0 ][ 1 ], dir[ 1 ][ 0 ], dir[ 1 ][ 1 ] );
		...
		if ( brush.rgrad )
		{
			var src = brush.rgrad,
				...
				r = src.r,
				bounds = src.bounds;
			var grad = bounds ? ctx.createRadialGradient( bounds[ 0 ] + dir[ 0 ][ 0 ] * bounds[ 2 ], bounds[ 1 ] + dir[ 0 ][ 1 ] * bounds[ 3 ], Math.max( bounds[ 2 ], bounds[ 3 ] ) * r[ 0 ], bounds[ 0 ] + dir[ 1 ][ 0 ] * bounds[ 2 ], bounds[ 1 ] + dir[ 1 ][ 1 ] * bounds[ 3 ], Math.max( bounds[ 2 ], bounds[ 3 ] ) * r[ 1 ] ) : ctx.createRadialGradient( dir[ 0 ][ 0 ], dir[ 0 ][ 1 ], r[ 0 ], dir[ 1 ][ 0 ], dir[ 1 ][ 1 ], r[ 1 ] );
			for ( var i = 0, slen = stops.length; i < slen; i++ )
			{
				var stop = stops[ i ];
				grad.addColorStop( stop[ 0 ], stop[ 1 ] )
	...
	function Sheet( src, callback, start_region )
	{
		...
		this.src = src;
		this.dimen = [ 0, 0 ];
		this.regions = [ [ 0, 0, 1, 1 ] ];
		this.regions_f = null;
		this.cur_region = start_region || 0;
		this.ready = false;
		this._image = null;
	...
	{
		if ( this._image ) throw new Error( "Already loaded" );
	...
	Sheet.prototype._drawToCache = function()
	{
		...
		var _canvas = newCanvas( this.dimen, 1 );
	...
	Sheet.prototype.apply = function( ctx )
	{
		...
		if ( this.region_f )
		{
			region = this.region_f( this.cur_region )
		}
		else
		{
			var r = this.regions[ this.cur_region ],
				d = this.dimen;
			region = [ r[ 0 ] * d[ 0 ], r[ 1 ] * d[ 1 ], r[ 2 ] * d[ 0 ], r[ 3 ] * d[ 1 ] ]
		}
		this._active_region = region;
		ctx.drawImage( this._cvs_cache, region[ 0 ], region[ 1 ], region[ 2 ], region[ 3 ], 0, 0, region[ 2 ], region[ 3 ] )
	};
	...
	Controls._TS = Controls._BH * 1.1;
	Controls._TW = Controls._TS * 4.4;
	Controls._SW = 1.3;
	...
	Controls.prototype.update = function( parent )
	{
		var _ratio = parent.__pxRatio,
			...
			_pp = find_pos( parent ),
			_bp = [ _pp[ 0 ] + parent.clientLeft, _pp[ 1 ] + parent.clientTop + _hdiff ],
			_cp = this._inParent ? [ parent.parentNode.offsetLeft + parent.clientLeft, parent.parentNode.offsetTop + parent.clientTop + _hdiff ] : _bp;
		...
		if ( !_canvas )
		{
			...
			if ( parent.id )
			{
				_canvas.id = "__" + parent.id + "_ctrls"
			}
		...
		if ( !this.ready )
		{
			var appendTo = this._inParent ? parent.parentNode : document.body;
			if ( this._inParent )
			{
		...
		this.bounds = [ _bp[ 0 ], _bp[ 1 ], _bp[ 0 ] + _w * _ratio, _bp[ 1 ] + _h * _ratio ]
	};
	...
	Controls.prototype.render = function( state, time )
	{
		...
		if ( !this.__force && time === this._time && _s === this._lhappens ) return;
		this._time = time;
		this._lhappens = _s;
		var ctx = this.ctx,
			_ratio = this._ratio;
		...
		if ( !this.__bggrad && !Controls.FLAT )
		{
			var bggrad = ctx.createLinearGradient( 0, 0, 0, _h ),
				bgspec = get_rgb( back );
			bggrad.addColorStop( 0, to_rgba( Math.min( bgspec[ 0 ] + 120, 255 ), Math.min( bgspec[ 1 ] + 120, 255 ), Math.min( bgspec[ 2 ] + 120, 255 ), .7 ) );
			bggrad.addColorStop( .35, to_rgba( Math.min( bgspec[ 0 ] + 30, 255 ), Math.min( bgspec[ 1 ] + 30, 255 ), Math.min( bgspec[ 2 ] + 30, 255 ), .8 ) );
			bggrad.addColorStop( .75, to_rgba( bgspec[ 0 ], bgspec[ 1 ], bgspec[ 2 ], .9 ) );
			bggrad.addColorStop( 1, to_rgba( bgspec[ 0 ], bgspec[ 1 ], bgspec[ 2 ] ) );
		...
		Controls.__progress( ctx, _pw, front, back, time, state.duration );
		ctx.translate( _pw + _m * 2.5, 0 );
		Controls.__time( ctx, front, back, this.elapsed ? time - state.duration : time );
		ctx.restore();
	...
	{
		( this._inParent ? parent.parentNode : document.body ).removeChild( this.canvas )
	};
	Controls.prototype.handle_mdown = function( event )
	{
		...
		else if ( _lx < _w - ( _tw + _m ) )
		{
			var _pw = _w / _ratio - ( _m * 5 + _tw + _bh ),
				_px = _lx - ( _bh + _m * 3 + Controls._SW ),
				_d = this.player.state.duration;
			var _tpos = _px / ( _pw / _d );
			if ( _tpos < 0 ) _tpos = 0;
			if ( _tpos > _d ) _tpos = d;
			if ( _s === C.PLAYING )
			{
			...
			else if ( _s === C.PAUSED || _s === C.STOPPED )
			{
	...
	Controls.prototype.inBounds = function( point )
	{
		...
		var _b = this.bounds;
		return point[ 0 ] >= _b[ 0 ] && point[ 0 ] <= _b[ 2 ] && point[ 1 ] >= _b[ 1 ] && point[ 1 ] <= _b[ 3 ]
	};
	...
	Controls.__time = function( ctx, front, back, time )
	{
		...
		var bgspec = get_rgb( back ),
			darkback = to_rgba( Math.max( bgspec[ 0 ] - 30, 0 ), Math.max( bgspec[ 1 ] - 30, 0 ), Math.max( bgspec[ 2 ] - 30, 0 ), .9 );
		ctx.save();
		Controls.__separator( ctx, 0, 0, front, darkback );
		ctx.fillStyle = front;
		ctx.textBaseline = "top";
		ctx.fillText( ( time < 0 ? "-" : "" ) + ( _h < 10 ? "0" + _h : _h ) + ":" + ( _m < 10 ? "0" + _m : _m ) + ":" + ( _s < 10 ? "0" + _s : _s ), _sw + _mr, 0 );
		ctx.restore()
	};
	...
	InfoBlock.prototype.update = function( parent )
	{
		var _ratio = parent.__pxRatio,
			...
			_pp = this._inParent ? [ parent.parentNode.offsetLeft + parent.clientLeft, parent.parentNode.offsetTop + parent.clientTop ] : find_pos( parent ),
	...
	InfoBlock.prototype.render = function()
	{
		...
		Text._ensureHasBuffer();
		var _tl = new Text( meta.title || "[No title]", "bold " + 12 * ratio + "px " + InfoBlock.FONT,
			{
				color: this.__fgcolor
			} ),
			_bl = new Text( ( meta.author || "[Unknown]" ) + " " + duration + "s" + " " + ( anim.width || 0 ) + "x" + ( anim.height || 0 ), 9 * ratio + "px " + InfoBlock.FONT,
			{
				color: this.__fgcolor
			} ),
			_p = InfoBlock.PADDING,
	...
	Errors.P.ALREADY_PLAYING = "Player is already in playing mode, please call " + "'stop' or 'pause' before playing again";
	...
	Errors.P.NO_STATE = "There's no player state defined, nowhere to draw, " + "please load something in player before " + "calling its playing-related methods";
	Errors.P.NO_SCENE = "There's nothing at all to manage with, " + "please load something in player before " + "calling its playing-related methods";
	Errors.P.COULD_NOT_LOAD_WHILE_PLAYING = "Could not load any scene while playing or paused, " + "please stop player before loading";
	...
	var to_export = {
		...
		{},
		obj_clone: obj_clone,
		createPlayer: function( cvs, opts )
		{
			var p = new Player;
			p.init( cvs, opts );
			return p
...
anm.__inject( "anm", typeof window !== "undefined" ? window : exports );
! function()
{
	var C = anm.C;
	...
	{
		var rectsMatched;
		if ( rectsMatched = G.__isecRects( this.rect( t ), elm.rect( t ) ) )
		{
			if ( !opts.pathDriven ) return true;
			var cx = this.xdata,
				ex = elm.xdata;
			...
			{
				return G.__pointsInPath( this.__pathAt( t ), elm.__pointsAt( t ) ) || G.__pointsInPath( elm.__pathAt( t ), this.__pointsAt( t ) )
			}
			else if ( pathOfC && !pathOfE )
			{
				var e_rect = elm.rect( t );
				return G.__pointsInRect( this.__pointsAt( t ), e_rect ) || G.__pointsInPath( this.__pathAt( t ), e_rect )
			}
			else if ( !pathOfC && pathOfE )
			{
				var c_rect = this.rect( t );
				return G.__pointsInRect( elm.__pointsAt( t ), c_rect ) || G.__pointsInPath( elm.__pathAt( t ), c_rect )
			}
			return false
		}
	};
	...
	E.___vecErrText = "Ensure you have passed actual " + "(current) time to _getVects " + "or check if vectorSpan value is > (1 / min.framerate) " + "or may be framerate itself is too low";
	E.prototype._makeGhost = function( t )
	{
		...
		{
			var s = this.__modifying !== null ? this._state : this.state,
				sn0 = s.snap0,
				sn1 = s.snap1;
			...
			else
			{
				...
				if ( pass_t <= opts.vectorSpan )
				{
					...
					s0 = E._stateBetween( sn0, sn1, span_t, span_pos );
					s1 = s;
	...
	E.prototype._defCollides = function( t, elm, func )
	{
		...
		elm.state = cmp_ghost;
		var intersects = this.intersects( elm );
		this.state = prev_src_st;
		elm.state = prev_cmp_st;
		if ( intersects )
		{
			func.call( this, t, src_ghost._tdiff, src_ghost._vec, cmp_ghost._vec )
		}
	};
	...
	E.prototype._cpa_bounds = function()
	{
		var cpath = this.xdata.__cpath;
		return cpath ? cpath.bounds() : this.lbounds()
	};
	E.prototype._cpa_rect = function()
	{
		var cpath = this.xdata.__cpath;
		return cpath ? cpath.rect() : this.lrect()
	};
	E.prototype.__ensureTimeTestAllowedFor = function( t )
	{
		if ( t != null && this.__modifying != null && this.__modifying !== E.EVENT_MOD )
		{
	...
	E.prototype.__mafter = function( t, type, result )
	{
		...
		if ( result && type === E.LAST_MOD )
		{
			...
			if ( colTests && colTests.length > 0 )
			{
				...
				for ( var ci = 0, cl = colTests.length; ci < cl; ci++ )
				{
	...
	E.prototype._getVects = function( t )
	{
		...
		else
		{
			...
			return {
				mov: [ s0.x + ( s1.x - s0.x ) * span_pos, s0.y + ( s1.y - s0.y ) * span_pos, s1.x + ( s.x - s1.x ) * pass_t, s1.y + ( s.y - s1.y ) * pass_t, vec_t ],
				rot: [ s0.angle + ( s1.angle - s0.angle ) * span_pos, s1.angle + ( s.x - s1.x ) * pass_t, s1.y + ( s.y - s1.y ) * pass_t, vec_t ],
				scl: [ s0.sx + ( s1.sx - s0.sx ) * span_pos, s0.sy + ( s1.sy - s0.sy ) * span_pos, s1.sx + ( s.sx - s1.sx ) * pass_t, s1.sy + ( s.sy - s1.sy ) * pass_t, vec_t ]
			}
		}
					...
					case C.X_MDOWN:
						{
							this.visitElems( function( elm )
							{
								if ( elm.visible && elm.contains( evt.pos ) ) elm.fire( type, evt )
							} );
							return true
						}
					case C.X_MMOVE:
	...
	Path.prototype.crosses = function( pt )
	{
		...
		if ( pt[ 0 ] != start[ 0 ] && pt[ 1 ] != start[ 1 ] )
		{
			crossings += G.__lineCrosses( pt[ 0 ], pt[ 1 ], cur[ 0 ], cur[ 1 ], start[ 0 ], start[ 1 ] )
		}
		return crossings
	};
	...
	{
		var pts = this.pts;
		return G.__lineCrosses( point[ 0 ], point[ 1 ], start[ 0 ], start[ 1 ], pts[ 0 ], pts[ 1 ] )
	};
	...
	CSeg.prototype.crosses = function( start, point )
	{
		var level = level || 0,
			pts = this.pts;
		return G.__curveCrosses( point[ 0 ], point[ 1 ], start[ 0 ], start[ 1 ], pts[ 0 ], pts[ 1 ], pts[ 2 ], pts[ 3 ], pts[ 4 ], pts[ 5 ], 0 )
	};
	var G = {};
	...
	{
		return r[ 0 ] === r[ 1 ] && r[ 1 ] === r[ 2 ] && r[ 2 ] === r[ 3 ] && r[ 3 ] === r[ 4 ] && r[ 4 ] === r[ 5 ] && r[ 5 ] === r[ 6 ] && r[ 6 ] === r[ 7 ]
	};
	...
	G.__inRect = function( r, pt, zeroTest )
	{
		...
		return pt[ 0 ] >= r[ 0 ] && pt[ 0 ] <= r[ 2 ] && pt[ 1 ] >= r[ 1 ] && pt[ 1 ] <= r[ 5 ]
	};
	...
	G.__isecRects = function( r1, r2 )
	{
		...
		for ( var pi = 0, pl = r1.length; pi < pl; pi += 2 )
		{
			pn = pi === pl - 2 ? 0 : pi + 2;
			px = pn === pl - 2 ? 0 : pn + 2;
			if ( edgeTest( [ r1[ pi ], r1[ pi + 1 ] ], [ r1[ pn ], r1[ pn + 1 ] ], [ r1[ px ], r1[ px + 1 ] ], r2 ) ) return false
		}
		for ( var pi = 0, pl = r2.length; pi < pl; pi += 2 )
		{
			pn = pi === pl - 2 ? 0 : pi + 2;
			px = pn === pl - 2 ? 0 : pn + 2;
			if ( edgeTest( [ r2[ pi ], r2[ pi + 1 ] ], [ r2[ pn ], r2[ pn + 1 ] ], [ r2[ px ], r2[ px + 1 ] ], r1 ) ) return false
		}
		return true
	};
	...
	G.__curveCrosses = function( px, py, x0, y0, xc0, yc0, xc1, yc1, x1, y1, level )
	{
		var level = level || 0;
		...
		return G.__curveCrosses( px, py, x0, y0, xc0, yc0, xc0m, yc0m, xmid, ymid, level + 1 ) + G.__curveCrosses( px, py, xmid, ymid, xmc1, ymc1, xc1, yc1, x1, y1, level + 1 )
	}
}();
! function()
{
...
window.Builder = function()
{
	...
	var modCollisions = C.MOD_COLLISIONS;
	var __b_cache = {};
	...
	{
		return is.arr( val ) ? Builder.path( val ) : val instanceof Path ? val : Path.parse( val, join )
	};
	...
	Builder.prototype.rect = function( pt, rect )
	{
		...
		var w = rect[ 0 ],
			h = rect[ 1 ];
		this.path( [ [ 0, 0 ], [ w, 0 ], [ w, h ], [ 0, h ], [ 0, 0 ] ], pt );
		return this
	};
	Builder.prototype.circle = function( pt, radius )
	{
		...
		{
			var b = this.$.__b$;
			Path.applyF( ctx, b.f, b.s, function()
			{
				ctx.arc( 0, 0, radius, 0, Math.PI * 2, true )
			} )
		} );
		if ( modCollisions ) this.v.reactAs( Builder.arcPath( 0, 0, radius, 0, 1, 12 ) );
		return this
	};
	Builder.prototype.image = function( pt, src, callback )
	{
		...
		{
			var b = this;
			this.x.sheet = new Image( src, callback )
		}
		return this
	};
	...
	{
		this.x.pos = pt;
		var text = lines instanceof Text ? lines : new Text( lines, Builder.font( font, size ) );
		this.x.text = text;
		this.x.reg = [ 0, 0 ];
		if ( !text.stroke )
		{
	...
	Builder.prototype.stroke = function( sval, width, cap, join )
	{
		this.s = is.str( sval ) ?
		{
			width: width != null ? width : Builder.DEFAULT_STROKE.width,
			color: sval,
	...
	{
		var x = this.x;
		x.pos = [ x.pos[ 0 ] + pt[ 0 ], x.pos[ 1 ] + pt[ 1 ] ];
		return this
	};
	...
	{
		return this.scale( band, [ [ values[ 0 ], values[ 0 ] ], [ values[ 1 ], values[ 1 ] ] ], easing )
	};
	Builder.prototype.trans = function( band, points, easing )
	{
		return this.transP( band, [ [ points[ 0 ][ 0 ], points[ 0 ][ 1 ] ], [ points[ 1 ][ 0 ], points[ 1 ][ 1 ] ] ], easing )
	};
	...
	Builder.prototype.disable = function()
	{
		...
		this.v.travelChildren( function( elm )
		{
			elm.disabled = true
		} );
		return this
	...
	Builder.fhsv = function( h, s, v, a )
	{
		...
		return Builder.rgb( 255 * ( rgb[ 0 ] + m ), 255 * ( rgb[ 1 ] + m ), 255 * ( rgb[ 2 ] + m ), a )
	};
	...
	Builder.path = function( points )
	{
		...
		for ( ; i < pl; i++ )
		{
			...
			{
				p.add( new CSeg( [ pts[ 0 ], pts[ 1 ], pts[ 2 ], pts[ 3 ], pts[ 4 ], pts[ 5 ] ] ) )
			}
		}
		return p
	};
	...
	Builder.easingF = Builder.easing;
	Builder.easingP = function( path )
	{
		return {
			type: C.E_PATH,
			data: Builder.__path( path )
		}
	};
	Builder.easingC = function( seg )
	{
		return {
			type: C.E_CSEG,
			data: seg instanceof CSeg ? seg : new CSeg( seg )
		}
	};
	...
	Builder.sheet = function( src, tile_spec, callback )
	{
		var sheet = new Sheet( src, function( img, sheet )
		{
			if ( is.arr( tile_spec ) )
			{
				...
				sheet.region_f = function( n )
				{
					var v_pos = Math.floor( n / h_factor ),
						h_pos = n % h_factor;
		...
		return function( elm )
		{
			return new _Animator( sheet, elm )
		}
	};
