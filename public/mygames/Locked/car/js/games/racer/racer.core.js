define( [ 'games/racer/util', 'games/racer/common' ], function( Util, Common )
{
	...
	function lastY()
	{
		return ( Common.segments.length == 0 ) ? 0 : Common.segments[ Common.segments.length - 1 ].p2.world.y;
	}
	...
	racer.addSprite = function( n, sprite, offset, collidable )
	{
		...
		Common.segments[ n ].sprites.push(
		{
			source: sprite,
			x: offset,
	...
	function resetRoad()
	{
		...
		for ( var n = 0; n < Common.rumbleLength * 3; n++ )
		{
			...
			var colors = jQuery.extend(
                {}, rumble % 2 === 0 ? Common.COLORS.FINISH : Common.COLORS.START );
	...
	function resetSprites()
	{
		...
		for ( n = 14; n <= 180; n += 40 )
		{
			racer.addSprite( n, Util.randomChoice( Common.SPRITES.BILLBOARDS ), -1.1 );
		}

		racer.addSprite( 240, Util.randomChoice( Common.SPRITES.BILLBOARDS ), -1.1 );
		racer.addSprite( 240, Util.randomChoice( Common.SPRITES.BILLBOARDS ), 1.1 );
		racer.addSprite( Common.segments.length - 25, Util.randomChoice( Common.SPRITES.BILLBOARDS ), -1.1 );
		racer.addSprite( Common.segments.length - 25, Util.randomChoice( Common.SPRITES.BILLBOARDS ), 1.1 );

		for ( n = 10; n < 200; n += 4 + Math.floor( n / 100 ) )
		{
			racer.addSprite( n, Util.randomChoice( Common.SPRITES.PLANTS ), 0.9 + Math.random() * 0.5 );
			racer.addSprite( n, Util.randomChoice( Common.SPRITES.PLANTS ), 1.1 + Math.random() * 2 );
		}

		for ( n = 250; n < 1000; n += 5 )
		{
			racer.addSprite( n, Util.randomChoice(Common.SPRITES.PLANTS), 1.2 );
			racer.addSprite( n + Util.randomInt( 0, 5 ), Util.randomChoice(Common.SPRITES.PLANTS), -1.2 - ( Math.random() * 2 ) );
			racer.addSprite( n + Util.randomInt( 0, 5 ), Util.randomChoice(Common.SPRITES.PLANTS), -1.2 - ( Math.random() * 2 ) );
		}
        ...
        {
            side = Util.randomChoice( [ 1.2, -1.2 ] );
        ...
        for ( n = 20; n < ( Common.segments.length - 40 ); n += 200 )
        {
            racer.addSprite( n, Util.randomChoice( Common.SPRITES.GANTRYS ), -0.37);
        }
	}
