...
(function (global) {
  ...
  class Physics {
    ...
    _drawPram(ctx) {
      ...
      this._drawWheel(ctx, this.frontWheel, 0.18, "front");
      this._drawWheel(ctx, this.rearWheel, 0.25, "rear");
    }

    _drawWheel(ctx, body, scale, key) {
      const w = img("pram_wheel");
      const p = body.getPosition();
      const cx = p.x * PS, cy = p.y * PS;
      const rPx = (w.naturalWidth / 2) * scale;     // drawn (ground-contact) radius
      // Roll the wheel by the distance it has TRAVELLED, not the physics body's spin.
      // The wheel bodies slip badly under the impulse "engine" and saturate Box2D's
      // per-step rotation cap (90 deg/step); on this 16-spoke wheel 90 deg is exactly
      // 4x the spoke symmetry, so the front wheel rendered frozen and the rear strobed
      // backwards. Travel-based rolling always reads correctly and tracks real speed.
      if (!this._wheelVis) this._wheelVis = {};
      const st = this._wheelVis[key] || (this._wheelVis[key] = { px: cx, py: cy, a: 0 });
      const dx = cx - st.px, dy = cy - st.py;
      const dTheta = (Math.sign(dx) * Math.hypot(dx, dy)) / rPx;   // radians this frame
      st.a += dTheta; st.px = cx; st.py = cy;
      // Motion blur: at speed, a 16-spoke wheel sampled at 30fps strobes, so trail a
      // few faint ghost copies across the arc swept this frame. The tyre/hub are
      // rotation-symmetric (stay solid); only the spokes smear. Slow wheels (a single
      // copy) stay crisp.
      const copies = Math.max(1, Math.min(6, Math.round(Math.abs(dTheta) * 180 / Math.PI / 12)));
      // Equal-weight copies spread across the swept arc: the rotation-symmetric tyre/
      // hub stay solid while the spokes smear evenly, so no single spoke aliases (which
      // is what read as "frozen"/"backwards"). A single copy (slow) stays fully crisp.
      for (let i = 0; i < copies; i++) {
        ctx.save();
        ctx.globalAlpha = 1 / copies;
        ctx.translate(cx, cy);
        ctx.rotate(st.a - dTheta * (i / copies));
