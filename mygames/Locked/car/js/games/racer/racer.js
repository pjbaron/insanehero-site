define( [ 'libs/hardcore', 'games/racer/racer.core', 'games/racer/util', 'games/racer/common', 'games/racer/humanPlayer' ], function( Animatron, Core, Util, Common, humanModule ){
    ...
    var background, road, sprites;
    var b = Builder._$;

    // road surface definitions (constant)
    var surfaceWide = 512;
    var surfaceHigh = 512;
    var rumbleWidth = 0;
    var numSlices = 16;
    ...
    var Game = {
        ...
        run: function( ai )
        {
            ...
            Game.loadImages( [ "background", "road", "sprites0", "sprites1", "sprites2", "sprites3" ], function( images )
            {
                background = images[ 0 ];
                road = images[ 1 ];
                sprites = [];
        ...
        { // load multiple images and callback when ALL images have loaded
            ...
            {
                var name = names[ n ];
                console.log("loadImages starting to load " + name);
        ...
        gameLoop: function( t )
        {
            ...
            { // Hack to start game (start it when t = 3)
    ...
    function getCarSprite( car, updown )
    {
        ...
        {
            var tilt = (Util.clamp(-8, updown + 8, 8) + 8.0) / 16.0;        // 0..1 inclusive
            updown = Math.floor(tilt * (Common.SPRITES.CAR_ORIENT.length - 1));
            var lrOrient;
            if ( car.isYou )
            {
                var dx = (car.car.dx / 0.03) * Common.SPRITES.CAR_MIDDLE_FRAME_INDEX;
                ...
                else
                {
                    ...
                    if ( lrOrient < 0.1 )
                        ...
                        lrOrient = Util.clamp( 0, Math.ceil( lrOrient ), Common.SPRITES.CAR_MIDDLE_FRAME_INDEX ) * -Util.getSign( dx );
                }
            }
            else
            {
                var xOff = ( car.car.x + 1 ) - ( player.car.x + 1 );
                lrOrient = Math.abs( xOff ) < 0.1 ? 0 :
                    xOff > 0 ? -2 : 2;
                //TODO: Calculate other cars' angles relative to you
            }
            sprite = Common.SPRITES.CAR_ORIENT[ updown ][ lrOrient + Common.SPRITES.CAR_MIDDLE_FRAME_INDEX ];
        }
        return sprite;
    }
    ...
    function attachInputs( parentScene )
    {
        ...
        parentScene.on( anm.C.X_MDOWN, function( evt )
        {
            mdown = true;
            if ( evt.pos[1] < height / 2)
            {
                input.faster = true;
                input.slower = false;
            }
            else if ( evt.pos[1] > height / 3 * 2)
            {
                input.slower = true;
                input.faster = false;
            }
            ...
            if ( evt.pos[0] < width / 3)
            {
                input.left = true;
                input.right = false;
            }
            else if ( evt.pos[0] > width / 3 * 2)
            {
                input.right = true;
                input.left = false;
            }
    ...
    function updateResults()
    {
        ...
        tmp = tmp.filter( function( n )
        {
            return ( typeof n !== 'undefined' );
        } ); // remove undefined (players that haven't finished yet) elements
    ...
    function buildScene()
    {
        ...
        var hudWidth = Math.max( 200, canvas.width * 0.16 );
        var hudHeight = 200;
        var labelHeight = 15;
        var valueHeight = 30;
        var margin = 4;
        ...
        var resultWidth = canvas.width * 0.5,
            resultHeight = canvas.height * 0.5;
        ...
        Core.update( Common.step,
        {} ); // one last update to draw the new stuff
    ...
    var Render = {
        ...
        segment: function( ctx, index, x1, y1, w1, x2, y2, w2, fog, color )
        {
            w1 *= 2;
            w2 *= 2;
            var dy = y2 - y1;
            var dx = w2 * 2 - w1 * 2;
            var slopeWidth = dx / dy;
            var slopex = (x2 - x1) / dy;
            var slopeOffset = (surfaceHigh / numSlices) / -dy;
            var offset = (index % numSlices) * (surfaceHigh / numSlices);
            var sx = Math.round(x1);
            var wide = Math.round(w1 + rumbleWidth) * 2;

            ctx.fillStyle = color.grass;
            ctx.fillRect( 0, y2, width, y1 - y2 );

            for (var sy = Math.ceil(y1); sy >= Math.floor(y2); sy--)
            {
                // img, sx, sy, sw, sh, x, y, w, h
                var srcy = Math.floor(offset) & (surfaceHigh - 1);
                ctx.drawImage(road, 0, srcy, surfaceWide, 1, sx - wide / 2, sy, wide, 1);
                sx -= slopex;
                wide -= slopeWidth;
                offset += slopeOffset;
            }
...
*/
        ...
        sprite: function( ctx, sprite, scale, destX, destY, offsetX, offsetY, clipY )
        {
            if (sprite === undefined)
            {
                console.log("can't draw an undefined sprite!");
                return;
            }

            var spriteScale = sprite.s;
            ...
            if ( clipH < destH )
                ctx.drawImage( sprites[sprite.i], sprite.x, sprite.y, sprite.w, sprite.h - ( sprite.h * clipH / destH ), destX, destY, destW, destH - clipH );
    ...
    function render( ctx )
    {
        ...
        Render.background( ctx, Common.BACKGROUND.SKY, skyOffset, resolution * skySpeed * playerY + Common.BACKGROUND.SKYOFFSET );
        ...
        for ( n = ( Common.drawDistance - 1 ); n > 0; n-- )
        {
            ...
            for ( i = 0; i < segment.sprites.length; i++ )
            {
                ...
                Render.sprite( ctx, sprite.source, spriteScale, spriteX, spriteY, sprite.x, -1, segment.clip );
            }
        }
    }
    return Game;
