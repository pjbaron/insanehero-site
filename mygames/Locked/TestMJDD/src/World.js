...
World.prototype.create = function( _managers, _level )
{
    ...
    World.SelectionPreviewX = 504;      //0.44;
	World.SelectionPreviewY = 206;      //0.23;
	World.SelectionPreviewBotY = 216;   //0.24;
...
World.prototype._createUI = function()
{
	...
	b.create( Main.topUI, "pause_button", this.managers, 516, 63, false,
	...
	var s = new Sprite();
	s.create( Main.topUI, "time_icon", this.managers.textures, -540, 63, false );
	s.anchor.x = s.anchor.y = 0.5;
	s.scale.x = s.scale.y = 1.0;
	this.icons.push(s);

	var t = new Text( "", Main.textStyleSemiBold );
	t.create( Main.topUI, -480, 63, false );
	t.anchor.y = 0.5;
	this.texts.push(t);
	this.timeText = t;

	s = new Sprite();
    s.create( Main.topUI, "score_icon", this.managers.textures, -150, 63, false );
    ...
    t.create( Main.topUI, -90, 63, false );
	...
	b = new Button( Button.TYPE_BUTTON );
	b.create( Main.bottomUI, "rotate_lft_button", this.managers, -0.43, 0.45, true,
		"rotate_lft_button", "rotate_lft_button_over", "rotate_lft_button", "click_left_event" );
	b.anchor.x = b.anchor.y = 0.5;
	b.scale.x = b.scale.y = 0.30;
	b.sfxHover = "snd_rollOver";
	//b.responsiveLayout = [ { minWidth: 800, position: { x: -0.30, y: 0.43 } } ];
	this.buttons.push(b);

	b = new Button( Button.TYPE_BUTTON );
	b.create( Main.bottomUI, "rotate_rgt_button", this.managers, 0.43, 0.45, true,
	...
	sprite = new Sprite();
	sprite.create( Main.topUI, "preview_container", this.managers.textures, World.SelectionPreviewX, World.SelectionPreviewY, false );
	...
	sprite = new Sprite();
	sprite.create( Main.topUI, "cube_s1_1", this.managers.textures, World.SelectionPreviewX, World.SelectionPreviewY, false );
	sprite.anchor.x = sprite.anchor.y = 0.5;
...
World.prototype._userInterface = function()
{
	...
	if ( this.container1 )
		this.container1.y = World.SelectionPreviewY;	
	if ( this.container2 )
